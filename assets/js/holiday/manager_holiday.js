...
//alert(maxDate.format("dd/mm/yy"));
   ...
   //var month = 4;      
   var year = year_new;
   var minDate = new Date(year-1, 3, 1);
   minDate = minDate.toLocaleDateString('en-GB'); //alert(minDate);
...
$(document).on('click', '#try', function(e) { 
            ...
            var user =$("#user").val(); 
            if(unit==0 || user=='none' || user==0)
            {
                swal('Please select unit and user');
                event.preventDefault(); 
            }
            else
            {
        ...
        else
        {
            ...
            { //alert('hii');
                ...
                $.ajax({ 
                    ...
                    success: function(data) { //console.log(data['leave_remaining']);
