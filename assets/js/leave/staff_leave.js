				...
				if(leave_date_array.length != new_date_array.length){
					...
					$('.div-warning-msg').hide();
						...
						$.ajax({ 
							...
							//url:baseURL+'staffs/Leave/findshifthours',
							url:baseURL+'staffs/Leave/findremainingleave',
							data: { 
								user_id:user_id,
							...
							success: function(data) {   //console.log(data);
								...
								// 	var num=(Math.round(totaldays * 100) / 100).toFixed(2); //console.log(num);
								// 	var res = num.replace(".", ":"); 
								// 	annual = annual.replace(":", ".");
								// 	remaining=annual-num;

								// 	total = $total.replace(":", "."); //console.log(total);
...
function returnDateWithoutHolidayAndOffday(start_date,end_date){
	...
	console.log(l_array_without_offday);
...
$(document).on('click', '#try', function(e) { 
			...
			var leave_remaining=parseFloat(leave_remaining);  
...
function editFeedback(id) {
        event.preventDefault(); // prevent form submit
          window.location = baseURL+"staffs/training/editFeedback/"+id;
    	...
    	function(isConfirm){
    	  if (isConfirm) {
    	   // form.submit();          // submitting the form when user press yes
    		  window.location = baseURL+"staffs/Leave/cancelleave/web/"+id
    	  } else {
    		  return false;
     	  }
    	});
