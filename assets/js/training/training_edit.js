$(function() {
    ...
    $(document).ready(function() {
      var selected_users_array = [];  
        ...
        if(new_status==2)
        {
          $( "#from-datepicker" ).datepicker( "option", "disabled", true );
          $( "#to-datepicker" ).datepicker( "option", "disabled", true );
        }
        else
        {
        ...
        $('#public-methods').multiSelect();  
          ...
          parent_unit=[];
          unitIds=unit; 
          for( var i = 0; i < user_new_array.length; i++ ) { 
            // var abc="user_"+user_new_array[i].user_id; //console.log(document.getElementById(abc));
            // document.getElementById(abc).checked = true;
            var user_name = user_new_array[i].fname+" "+user_new_array[i].lname;
            var object = user_new_array[i].user_id+"_"+user_new_array[i].unit_id+"_"+user_name+"_"+user_new_array[i].unit_shortname;
  ...
  $.ajax({
        ...
        var table = $('#myTable1').DataTable({ 
                                    ...
                                    initComplete: function () {
                                      ...
                                      columnDefs: [
                                        { orderable: false, targets: [2] }, // Disable sorting for the 3rd column (index 2)
                                      ],
                                      processing: true,
                                      language: {
                                      ...
                                      serverSide: true,
                                      ajax: {
                                        url: baseURL + 'admin/training/editdata_report',
                                        dataType: 'json',
                                        type: "POST",
                                        data: function (d) {
                                            // Include custom filters in the Ajax request data
                                            d.designation_filter = $('#hgr').val();
...
// Listen for the draw.dt event and check checkboxes after each draw
...
function checkCheckboxes() {
  // Add a slight delay to ensure that the checkboxes are present in the DOM
  setTimeout(function() {
      // Loop through the checkboxIds array
     
      unitIds=unit; 
      for( var i = 0; i < user_new_array.length; i++ ) { 
        var abc="user_"+user_new_array[i].user_id; //console.log(document.getElementById(abc));
        // document.getElementById(abc).checked = true;
          // Use jQuery to check the checkbox by setting the 'checked' attribute
 ...
 $(document).on('click', '.checkItem', function(){
    ...
    $(document).on('click', '.selectall', function(){
      ...
      if ($(this).is(':checked')) {
        ...
        if(designation=='')
        {   
          var abc=table.$("input[type='checkbox']").attr('checked', $(this.checked));  //console.log(abc);
          ...
          {  
              ...
              var new_values=user_details.split('___'); //console.log(user_details);
              // console.log(new_values)
              var unit_shortcode =new_values[3]; 
              var user_name = new_values[4]; 
              var unit_id = new_values[2];
              var user_id = new_values[0];
              var object = user_id+"_"+unit_id+"_"+user_name+"_"+unit_shortcode;              /*var object = {
