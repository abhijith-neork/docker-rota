...
class Rota extends CI_Controller {
  ...
  public function staffviewrota() //mobile view staff view rota
  {
   ...
   foreach ($response['holiday'] as $holiday) {
    ...
    $this->load->view("staffs/rota/staffviewrota",$response);
    //$result['js_to_load'] = array('fullcalendar/bootstrap.min.js','fullcalendar/moment.js','fullcalendar_staff.js','rota/staffviewrota.js');
    //$this->load->view('includes/staffs/newstaff_footerrota',$result);
  }

  public function staffmanagerota(){   //mobile view staff availability
      ...
      if(count($user_unit_details) > 0){
        ...
        $this->load->view("staffs/rota/staffmanagerota",$response);
        // $result['js_to_load'] = array('fullcalendar/bootstrap.min.js','fullcalendar/moment.js','fullcalendar/fullcalendar.js','fullcalendar/scheduler.js','fullcalendar/sweetalert2.min.js','staff_rota/rota.js');
        }
  ...
  public function index()
  {
   ...
   foreach ($response['holiday'] as $holiday) {
    ...
    $response['trainings']=json_encode($training); 
    $response['holiday']=json_encode($holiday); 
    $this->load->view("staffs/rota/viewrota",$response);
    $result['js_to_load'] = array('fullcalendar/bootstrap.min.js','fullcalendar/moment.js','fullcalendar_staff.js','rota/staffviewrota.js');
    $this->load->view('includes/staffs/footer_rota',$result);
  }
  ...
  public function getRota(){
      ...
      $unitDetails = $this->Unit_model->findunit($rota['unit_id']);
       ...
       if($rota['shift_name']=='X')
           $time = 'Offday';
               ...
               $time = $rota['start_time']."-".$rota['end_time'];
               $s =$this->multi_array_search($posts, array('rota_date' =>$rota['date'], 'resourceId' => $rota['user_id']));
               if(count($s)>0){
    ...
    //                "title"            =>  'Training', 
    //                "unit_name"        =>  $unitDetails[0]['unit_name'],
    //                "start"            =>  $training['date_from']."T".date("H:i", strtotime($training['time_from'])),
    //                "end"              =>  $training['date_to']."T".date("H:i", strtotime($training['time_to'])),
    //                "stime"=>$training['date_from'],"etime"=>$training['date_to'],"time"=>'Title' .'-'. $training['title'],
    //                "unit_color"=>$unitDetails[0]['color_unit']
...
/*     $response['holiday']=$this->Leave_model->getWeeklyHoliday($params);  
  ...
  function removeDuplicates($posts){
    ...
    public function getDayRota()
    {
        ...
        $month_diff = $this->input->post("month_diff");
        if($month_diff >= 2){
          $dayrota=$this->Staffrota_model->getDayRota($params);
        }else{
          $dayrota=$this->Shift_model->getDayRota($params);
        }
        if(count($dayrota)>0){
            $dayrotastatus=1;
        }
        ...
        // $response['userShifts']=json_encode($posts);
        header('Content-Type: application/json');
        echo json_encode(array('dayrotastatus'=>$dayrotastatus,"shiftdata"=>$dayrota));
        exit();
    ...
    public function createrota(){  
      ...
      if(count($user_unit_details) > 0){
        ...
        $this->load->view("staffs/rota/managerota",$response);
        $result['js_to_load'] = array('fullcalendar/bootstrap.min.js','fullcalendar/moment.js','fullcalendar/fullcalendar.js','fullcalendar/scheduler.js','fullcalendar/sweetalert2.min.js','staff_rota/availability_staff.js');
        }
        $this->load->view('includes/staffs/footer_rota',$result);
      }
        ...
        if($result['status'] == "true"){
          ...
          if(count($unit_supervisor)>0){
            ...
            $emailSettings = array(
                ...
                'to' => $supervisor_email,
                'type' => 'Staff-Open slot request accepted',
      ...
      if($result['status'] == "true"){
        ...
        if(count($unit_supervisor)>0){
          ...
          $emailSettings = array(
              ...
              'site_url' => $this->config->item('base_url'),
              'to' => $supervisor_email,
              'type' => 'Staff-Open slot request rejected',
              'staff_name' => $user_name,
              'supervisor_name'=>$supervisor_name,
              'subject' => $subject,
              'data' => $body,
  ...
  function getUserRequestCount(){
    ...
    $params['user_id']   = $this->input->post("user_id");
    $params['shift_id']= $this->input->post("shift_id");
    $params['start_date']= $this->input->post("cs_date");
    $params['end_date']= $this->input->post("ce_date");
    $request_dates = $this->input->post("request_array");
    $p_rota = $this->Rotaschedule_model->getPublishedRotaOfUser($params,$request_dates);
    if(count($p_rota) > 0){
      $final_result = $p_rota;
    }else{
      $final_result = $this->Staffrota_model->getUserRequestCount($params,$request_dates);
    }    
    header('Content-Type: application/json');
