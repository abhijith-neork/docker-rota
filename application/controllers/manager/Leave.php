...
class Leave extends CI_Controller {
    ...
    public function __construct() {
        ...
        $data['error'] =''; 
        $data['message'] ='';
        $this->load->view("manager/leave/listmobleavebymanager",$data);
    ...
    public function index() {
        ...
        // $leaveDataResult = $this->Leave_model->losOfPayChecking($this->session->userdata('user_id')); 
        // $annual_leave=$this->Leave_model->findAnnualHollidayAllowance($id); 
        // $actual_holiday_allowance=$annual_leave[0]['annual_holliday_allowance']; //print_r($actual_holiday_allowance);
        // $leave_remaining=$this->Leave_model->findleaveremainingbyuser($id);
        ...
        /*End*/ 
        $this->load->view("manager/leave/listleaves",$data);
    ...
    public function leaverequest(){
       ...
       // print_r($holidyTypeDetails['total_days']);exit(); 
        ...
        $this->load->view("manager/leave/leaverequest",$holidyTypeDetails);
        //$jsfile = array();
        $jsfile['js_to_load'] = array('leave/leave.js');
        $this->load->view('includes/staffs/footer',$jsfile);
    }
...
// Function call with passing the start date and end date 
    ...
    public function findshifthours()
    {
     ...
     $array_without_offday_holiday=$this->input->post('array_without_offday_holiday');
             ...
             {   
                 ...
                 { 
                    ...
                    if (in_array($day, $array_without_offday_holiday)) {
                        $targeted_hours=$this->Leave_model->findtargetedhours($params['user_id']); //finding targeted hour of user's default shift
                 ...
                 { 
                    if (in_array($day, $array_without_offday_holiday)) {
                 // print_r("shift-");
                    $shift_hours=$shift[0]['shift_hours']; 
...
//print $hours."H--";
    ...
    { //print_r('hii');exit();
         ...
         $leave=getHoursByUser($user_id,$leaveyear); //print_r($leave); print '<br>';
          if(date("m") > 8)
          {
              $currentyear = date("Y");
              $nxtyear = date("Y")+1;
        ...
        {   //print_r('hii');exit();
            ...
            //if((int)$leave_remaining <= 0){$leave_remaining='0.00';}else{$leave_remaining=number_format((float)$leave_remaining, 2, '.', '');}
        } 
        else 
        {  //print_r('hello');exit();
    ...
    public function applyLeave()
    {
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $holidyTypeDetails['user']=$this->User_model->findusers($this->session->userdata('user_id'));
            $selected_year=date('Y', strtotime('+1 years'));
            $holidyTypeDetails['selected_date']=$selected_year.'/'.'07'.'/'.'31';
            $holidyTypeDetails['error']='';
            $holidyTypeDetails['message'] =''; 
            $this->load->view("manager/leave/leaverequest",$holidyTypeDetails);
        }
        ...
        {  
            ...
            if($result['status'] == false){
                ...
                redirect('manager/leave/leaverequest');
            }  
            ...
            $leave_remaining=$this->Leave_model->checkLeaveremaining($id); //print_r($leave_remaining);exit();
                ...
                $annual_holiday_allowance=$this->Leave_model->getActualholidayallowance($id); //print_r($annual_holiday_allowance);exit();
...
*/                                   $sum=$sum+$calculated_hours;
                    ...
                    if($userUnitDetails['values'])
                    {
                        ...
                        // print_r($userUnitDetails['user_values']);exit();
                        if($userUnitDetails['user_values'])
                        {
                           ...
                           // print_r($already_approved_count);exit();
                            ...
                            if(count($supervisorDetails['values']) > 0)
                            {
                                    ...
                                    $emailSettings = array(
                                            ...
                                            'type' => 'Manager/Supervisor-apply leave',
            ...
            $id=$this->uri->segment(5);  
                    ...
                    $this->Leave_model->cancelleave($id);
                        ...
                        $this->session->set_flashdata('message', 'Leave canceled successfully.');
                            if($device=='mob')
                            { 
                              redirect('manager/leave/listleaves');
                            }
                            else
                            { 
                               redirect('manager/Leave');
