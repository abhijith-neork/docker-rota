...
<script type="text/javascript" src="<?php echo base_url();?>assets/plugins/styleswitcher/jQuery.style.switcher.js"></script>
 ...
 <script type="text/javascript">
    ...
    else
    {
        ...
        function sendAppPassword(id, fname) {
            event.preventDefault(); // prevent form submit
            //var form = event.target.form; // storing the form
            var appmsg,appstatus;
            
            appstatus = $("#apppassword option:selected").val();

            if(appstatus>0){
                        
                    console.log(appstatus);
                    if(appstatus==1){
                  	  appmsg = "Enable app password login to the user "+fname+"? This will send a 4 digit password to user\'s mobile.";
                    }
                    if(appstatus==2){
                  	  appmsg = "Disable app password login to the user "+fname+"?";
                    }
                            swal({
                      title: appmsg, 
                      type: "warning",
                    ...
                    if (isConfirm) {
                      $.ajax({
                        type: 'POST',
                        url: baseURL+"admin/user/sendapppassword",
